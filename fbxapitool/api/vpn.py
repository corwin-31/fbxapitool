class Vpn:
    ...
    vpnservers = [ 'ipsec', 'pptp', 'openvpn_routed', 'openvpn_bridge' ]
    ...
    def get_server_config(self, vpn_id):
        '''
        Gets a VPN Server configuration
        '''
        return self._access.get(f"vpn/{vpn_id}/config/")

    def set_server_config(self, vpn_id, glob_conf={}, spec_conf = None):
        '''
        ...
        return self._access.put(f"vpn/{vpn_id}/config/", glob_conf)

    def set_server_pptp_auth(self, vpn_id, conf = pptp_auths):
        '''
        ...
        return self._access.put(f"vpn/{vpn_id}/config/", { 'conf_pptp': { 'allowed_auth': conf } })
    ...
    def get_server_user(self, login):
        '''
        Gets a VPN Server user details
        '''
        return self._access.get(f"vpn/user/{login}")

    def add_server_user(self, conf):
        '''
        Adds a VPN Server user
        '''
        return self._access.post('vpn/user/', conf)

    def del_server_user(self, login):
        '''
        Deletes a VPN Server user
        '''
        return self._access.delete(f"vpn/user/{login}")

    def set_server_user(self, login, conf):
        '''
        Updates a VPN Server user
        '''
        return self._access.put(f"vpn/user/{login}", conf)

    def get_server_ippool(self):
        '''
        Gets VPN Server IP pool reservations
        '''
        return self._access.get('vpn/ip_pool/')
    ...
    def close_server_connection(self, id):
        '''
        Closes a VPN Server connection
        '''
        return self._access.delete(f"vpn/connection/{id}")

    def get_server_user_config(self, server_name, login):
        '''
        Gets an OpenVPN Server user configuration for OpenVPN
        '''
        return self._access.get(f"vpn/download_config/{server_name}/{login}")

    def get_client_list(self):
        '''
        Gets VPN Client list
        '''
        return self._access.get('vpn_client/config/')

    def get_client_config(self, vpn_id):
        '''
        Gets a VPN Client configuration
        '''
        return self._access.get(f"vpn_client/config/{vpn_id}")

    def add_client(self, conf, vpntype=vpnclient_types[0], descr=''):
        '''
        Adds a VPN Client
        '''
        ...
        return self._access.post('vpn_client/config/', payload=glob_conf)

    def del_client(self, id):
        '''
        Deletes a VPN Client
        '''
        return self._access.delete(f"vpn_client/config/{id}")

    def set_client_active(self, id):
        '''
        Changes the active VPN Client
        '''
        return self._access.put(f"vpn_client/config/{id}", { 'active': True })

    def set_client(self, id, conf, descr=None):
        '''
        ...
        return self._access.put(f"vpn_client/config/{id}", glob_conf)

    def set_client_pptp_auth(self, id, conf):
        '''
        ...
        if cur_conf['type'] != 'pptp': return None
        if cur_conf['conf_pptp']['mppe'] != self.pptp_mppes[0]: return None
        return self._access.put(f"vpn_client/config/{id}", { 'conf_pptp': { 'allowed_auth': conf } })

    def get_client_status(self):
        '''
        Gets VPN Client status
        '''
        return self._access.get('vpn_client/status/')

    def get_client_logs(self):
        '''
        Gets VPN Client logs
        '''
        return self._access.get('vpn_client/log/')
    
    def set_slavery(self, slave=True):
        '''
        Manages if download must only be done with a VPN client connection
        '''
        return self._access.put('vpn_client/apps/', { 'fbxgrabberd': { 'use_vpn': slave } })

    def get_slavery(self):
        '''
        Gets application (=downloads) slavery status
        '''
        return self._access.get('vpn_client/apps/')