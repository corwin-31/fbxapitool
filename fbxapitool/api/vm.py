...
class Vm:
    ...
    def get_config_all(self):
        ...
        '''
        return self._access.get('vm/')

    def get_config_vm(self, id):
        '''
        Gets VM #id configuration
        '''
        return self._access.get('vm/{0}/'.format(id))
    
    def start(self, id):
        '''
        Starts specified VM
        '''
        return self._access.post('vm/{0}/start'.format(id), payload=None)
    
    def restart(self, id):
        '''
        Restarts specified VM
        '''
        return self._access.post('vm/{0}/restart/'.format(id), payload=None)
    
    def stop(self, id):
        '''
        Stops specified VM
        '''
        return self._access.post('vm/{0}/powerbutton'.format(id), payload=None)

    def halt(self, id):
        '''
        Halt/force stop specified VM
        '''
        return self._access.post('vm/{0}/stop'.format(id), payload=None)

    def get_distrib(self):
        '''
        Gets all free distributions
        '''
        return self._access.get('vm/distros')
    
    def resize(self, vfile, size, shrink=False):
        '''
        Resize VM virtual file
        '''
        json = { 'disk_path': base64.b64encode(vfile.encode('utf-8')).decode('utf-8'), 'shrink_allow': shrink, 'size': size }
        return self._access.post('vm/disk/resize', payload=json)
    
    def get_task(self, id):
        '''
        Gets rezise task status
        '''
        return self._access.get('vm/disk/task/{0}'.format(id))
    
    def del_task(self, id):
        '''
        Removes rezise task
        '''
        return self._access.delete('vm/disk/task/{0}'.format(id))
    ...
    def create(self, data, decodedir=True):
        '''
        Creates a new VM
        '''
        if decodedir and 'disk_path' in data: data['disk_path'] = base64.b64encode(data['disk_path'].encode('utf-8')).decode('utf-8')
        return self._access.post('vm/', payload=data)
    
    def set_config(self, id, data, decodedir=True):
        '''
        Sets VM #id configuration
        '''
        if decodedir and 'disk_path' in data: data['disk_path'] = base64.b64encode(data['disk_path'].encode('utf-8')).decode('utf-8')
        return self._access.put('vm/{0}'.format(id), payload=data)
    
    def delete(self, id):
        '''
        Removes a VM
        '''
        return self._access.delete('vm/{0}'.format(id))
